...
//Nabilosat
#define ENABLE_SKY_EPG 1
#define ENABLE_FREESAT 1
#define ENABLE_NETMED 1
#define ENABLE_VIRGIN 1

//SKY_EPG require MHW_EPG structures
#if defined(ENABLE_SKY_EPG) && !defined(ENABLE_MHW_EPG)
#define ENABLE_MHW_EPG 1
#endif
...
#include <lib/dvb/lowlevel/eit.h> //needed by permanenttimeshift
...
//Nabilosat
		...
		ePtr<eConnection> m_SKYConn;
		ePtr<iDVBSectionReader> m_SKYReader;
		eDVBSectionFilterMask m_SKYFilterMask;
		...
		void storeSKYTitle(std::map<__u32, mhw_title_t>::iterator itTitle, std::string sumText, const __u8 *data);
		void storeWebTitle( mhw_title_t Title, std::string sumText, const __u8 *data);
#endif
//end
	};
