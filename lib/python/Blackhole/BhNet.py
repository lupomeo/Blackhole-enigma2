...
class DeliteOpenvpn(Screen):
	...
	def restartVpn(self):
		if self.my_vpn_active == False:
			mybox = self.session.open(MessageBox, _("Activate OpenVpn First"), MessageBox.TYPE_INFO)
			mybox.setTitle("Info")
...
class DeliteTelnet(Screen):
	...
	def __init__(self, session):
		...
		self["actions"] = ActionMap(["WizardActions", "ColorActions"],
		{
			"ok": self.close,
			"back": self.close,
			"red": self.NTelnetStop,
			"green": self.NTelnetStart
		})
