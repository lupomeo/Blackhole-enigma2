...
class BhRedWrong(Screen):
	...
	def __init__(self, session):
		...
		self["key_yellow"] = Label(_("Big Bang"))
...
class BhRedPanel(Screen):
	...
	def __init__(self, session):
		...
		self["key_red"] = Label(_("Exit"))
		self["key_green"] = Label(_("Jump to..."))
		self["key_yellow"] = Label(_("Big Bang"))
		self["lab1"] = Label()
		self["lab2"] = Label()
		...
		self["actions"] = ActionMap(["WizardActions", "ColorActions"],
		{
			...
			"ok": self.confiG,
			"red": self.close,
			"green": self.checkdesT,
			"yellow": self.check_origin
		})
...
# build bootup file
		...
		if self.destination != "Black Hole":
			if self.destination_force_reboot == "False":
				...
				out.write("#!/bin/sh\n\n")
				out.write("mount -o bind %s /etc > /tmp/jump.tmp\n" % (path1))
				out.write("mount -o bind %s /var/lib/opkg > /tmp/jump.tmp\n" % (path3))
				vumodel = getModel()
				if vumodel in ("uno4kse", "zero4k", "duo4k"):
					out.write("mkdir -p %s > /tmp/jump.tmp\n" % (pathw))
