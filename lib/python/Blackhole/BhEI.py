...
#		self["cam_info"].setText(self.currentCam)
		...
		if fileExists("/tmp/updatebool"):
			omb = False
			cnt = 0
			f = open("/tmp/updatebool",'r')
 			for line in f.readlines():
 				if len(line) > 4:
 					if line.find('Not selecting') == -1:
						cnt = cnt +1
				if line.find('openmultiboot') != -1:
					omb = True
			f.close()
			if cnt == 1 and omb == False:
				cnt = 2
			if cnt > 1:
				self["button_upg"].show()


#			upsize = getsize("/tmp/updatebool")
#			if upsize > 1:
#				self["button_upg"].show()
	...
	def parse_caid_txt(self, caid):
		ret = ""
		caidnames = ['0100,01FF,Seca', '0500,05FF,Viaccess', '0600,06FF,Irdeto', '0900,09FF,Videoguard', '0B00,0BFF,Conax', '0D00,0DFF,Cryptoworks', '1000,10FF,Tandberg', '1700,17FF,Betacrypt', '1800,18FF,Nagravision', '2600,26FF,Biss', '4AEE,4AEE,Bulcrypt', '5500,55FF,Bulcrypt', '2700,2710,Dre-crypt', '4AE0,4AE1,Dre-crypt', '0E00,0EFF,PowerVu', '5601,5604,Verimatrix']
...
#		return ret
	...
	def show_emm(self, caidname):
		if caidname == "Betacrypt":
			...
			self["cw_emm"].show()
		elif caidname == 'Dre-crypt':
			self['dre_emm'].show()
		elif caidname == "Videoguard":
			self["nds_emm"].show()
		elif caidname == 'PowerVu':
			self['pv_emm'].show()
		elif caidname == 'Verimatrix':
			self['vm_emm'].show()
