...
static const std::string getConfigCurrentSpinner(const std::string &key)
{
	...
	if (png.good()) {
		...
		return "skin_default/spinner";  // if value is NOT empty, means config.skin.primary_skin exist in settings file, so return "spinner" ( /usr/share/enigma2/MYSKIN/spinner/wait1.png DOES NOT exist )
} 
